...
document.getElementById("stockForm").addEventListener("submit", async function (event) {
  ...
  try {
    ...
    if (!dcfData || dcfData.length === 0 || !dcfData[0].enterpriseValue || !dcfData[0].price) {
      throw new Error("Invalid DCF data received from API.");
    }

    const targetPrice = (dcfData[0].enterpriseValue + dcfData[0].totalCash - dcfData[0].totalDebt) / dcfData[0].dilutedSharesOutstanding;
    const stockPrice = dcfData[0].price;

    resultDiv.innerHTML = `
      <p><strong>Stock Target Price for ${ticker.toUpperCase()}:</strong> $${targetPrice.toFixed(2)}</p>
      <p><strong>Current Stock Price for ${ticker.toUpperCase()}:</strong> $${stockPrice.toFixed(2)}</p>
    `;
    const chartResponse = await fetch(
      `https://financialmodelingprep.com/api/v3/historical-price-full/${ticker}?timeseries=30&apikey=${apiKey}`
    );
    if (!chartResponse.ok) {
      throw new Error(`Failed to fetch historical data: ${chartResponse.statusText}`);
    }
    ...
    const stdDev = Math.sqrt(closePrices.map(x => Math.pow(x - mean, 2)).reduce((a, b) => a + b, 0) / closePrices.length);
